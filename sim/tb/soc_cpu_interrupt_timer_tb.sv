...
module soc_cpu_interrupt_timer_tb;
    ...
    soc_core_top #(
        .ENABLE_CPU(1'b1),
        .BOOT_ROM_INIT_FILE("build/interrupt_timer.mem"),
    ...
    initial begin
        ...
        rst <= 1'b0;
        cycles = 0;
        while (dut.u_soc_cpu_cluster.u_dtcm.memory[3] !== 32'h5449_4d52 &&
               cycles < 10000) begin
            @(posedge clk);
            cycles++;
        end
        check(cycles < 10000,
              "CPU services external and timer interrupts through SoC MMIO");
        check(dut.u_soc_cpu_cluster.u_dtcm.memory[0] == 32'h8000_000B,
              "CPU records machine-external interrupt mcause");
        check(dut.u_soc_cpu_cluster.u_dtcm.memory[1] == 32'd1,
              "LIC claim returns Ethernet/source-1 interrupt ID");
        check(dut.u_soc_cpu_cluster.u_dtcm.memory[2] == 32'h8000_0007,
              "CPU records machine-timer interrupt mcause");
        check(dut.u_soc_cpu_cluster.u_dtcm.memory[3] == 32'h5449_4D52,
              "timer handler writes its completion signature");
        check(!dut.local_external_irq,
              "claim/complete plus disable clears the CPU external IRQ");
        check(!dut.machine_timer_irq,
              "timer handler raises mtimecmp and clears MTIP");
        check(dut.u_soc_cpu_cluster.u_cpu.u_cpu_wrapper.mtime_value ==
              dut.machine_time,
              "CPU time CSR is driven by the same SoC machine timer");
        check(!m_axi_awvalid && !m_axi_arvalid,
              "interrupt firmware local traffic does not escape to DRAM AXI");
        check(!mmio_ready,
              "external debug MMIO is isolated while internal CPU owns MMIO");
        $display("CPU interrupt/timer regression: %0d checks passed", checks);
        $finish;
    end
endmodule
